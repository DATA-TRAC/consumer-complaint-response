...
def unique_words(word_counts, rows, cols):
    ...
    for i, col in enumerate(word_counts.columns):
        plt.subplot(rows, cols, i+1)  # Adjust the subplot parameters as per your requirement

        # Sort the values in the column in descending order and select the top 5
        top_words = word_counts[col].sort_values(ascending=False).head(5)
...
def get_words2(train):
    ...
    #grabbing frequencies of occurrences
    relief_freq = pd.Series(relief_words.split()).value_counts()
    no_relief_freq = pd.Series(no_relief_words.split()).value_counts()
...
def get_words_products(train):
    ...
    #grabbing frequencies of occurrences
    credit_report_freq = pd.Series(credit_report_words.split()).value_counts()
    debt_freq = pd.Series(debt_words.split()).value_counts()
    credit_card_freq = pd.Series(credit_card_words.split()).value_counts()
    mortgage_freq = pd.Series(mortgage_words.split()).value_counts()
    loans_freq = pd.Series(loans_words.split()).value_counts()
    bank_freq = pd.Series(bank_words.split()).value_counts()
    money_service_freq = pd.Series(money_service_words.split()).value_counts()
    all_freq = pd.Series(all_words.split()).value_counts()
